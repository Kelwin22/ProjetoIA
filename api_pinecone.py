...
# from openai import OpenAI # <--- REMOVIDO: Importação da OpenAI
from dotenv import load_dotenv
from pydantic import BaseModel
from typing import List, Optional
import uvicorn
from pinecone import Pinecone
import google.generativeai as genai # <--- ADICIONADO: Importação do Gemini
import time # Importado para o sleep no conectar_pinecone
...
# GEMINI_CHAT_MODEL é lido em llm_router.py, mas é bom ter a referência aqui também.
GEMINI_CHAT_MODEL = os.getenv("GEMINI_CHAT_MODEL", "models/gemini-pro")
...
# REMOVIDO: A função gerar_embedding não deve estar aqui. Ela está no pinecone_utils.py.
...
# Importa as funções necessárias do pinecone_utils.py, incluindo gerar_embedding
# Certifique-se que pinecone_utils está acessível e possui as funções abaixo
from pinecone_utils import gerar_embedding # <--- ADICIONADO: Importando gerar_embedding
...
def listar_contratos(
    ...
    try:
        ...
        dummy_vector = [0.0] * 768 # <--- CORRIGIDO: Dimensão para Gemini embedding
...
def buscar_contratos(
    ...
    """
    Realiza uma busca semântica nos contratos usando o Pinecone com embeddings do Gemini.
    """
    global index
    ...
    try:
        # Gera o embedding da consulta usando a função gerar_embedding (agora do Gemini)
        query_embedding = gerar_embedding(q) # <--- Agora usa a função importada de pinecone_utils.py
